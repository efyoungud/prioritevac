...
to go ; master command to run simulation
 ...
 if ticks = 50 ; approximate time people started being directed out the kitchen exit
  ...
  if ticks = 130 [ask people [set alarmed? true]] ; 40 seconds after the Cabaret Room announcement all people are set to become alarmed. this manual input was added because full smoke coverage was not included to contribute to the organic alert function
end
